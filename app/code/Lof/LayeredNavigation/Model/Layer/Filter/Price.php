...
class Price extends AbstractFilter
{
	...
	public function apply(\Magento\Framework\App\RequestInterface $request)
	{
		...
		$priceSql = "SELECT entity_id FROM `catalog_product_index_price` WHERE `website_id` = '1' AND `customer_group_id` = '0' AND `min_price` >= '$from' AND `min_price` <= '".(empty($to) || $from == $to ? $to : $to - self::PRICE_DELTA)."'";

		$product_ids = array_map("intval",array_column($connection->fetchAll($priceSql),"entity_id"));
		...
		*/
		...
		//$this->getLayer()->getProductCollection()->addAttributeToFilter("entity_id", ['in' =>$product_ids]);
