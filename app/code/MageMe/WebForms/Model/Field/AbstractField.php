...
abstract class AbstractField extends AbstractModel implements FieldInterface
{
    ...
    {
        try {
            //preserve the {{var ...}} variables and apply simple filter
            $value = @$this->getFilter()->filter(str_replace('{{var','\{\{var',$value));
...
#region DB getters and setters
    ...
    public function getPostErrors(array $postData, bool $logicVisibility, array $config = []): array
    {
        ...
        // check required
        if ($this->getIsRequired()) {
            if ($this->_validatePostRequired($postData, $logicVisibility)) {
     ...
     * @return bool
     */
    protected function _validatePostRequired(array $postData, bool $logicVisibility): bool
    {
        ...
        return !($logicVisibility && ($value == $placeholder || $value == ''));
    }
    ...
    {
        return (bool)$this->getData(self::IS_LOGIC_TYPE);
    }
