...
define([
    ...
    'use strict';
    $.widget('mage.sellerAddProductjs', {
        options: {
            errorMessageSku: $t("SKU can\'t be left empty"),
            ajaxErrorMessage: $t('There was error during fetching results.'),
            eventdateMessage: $t('you can not select previous date from event start date')
        },
        _create: function () {
            var self = this;
            $("#form-seller-event-new").dateRange({
                'dateFormat':'mm/dd/yy',
                'from': {
            ...
            $("#event_end_date").datetimepicker({
                ...
                'minDate': new Date(),
                onClose: function ( selectedDate ) {
                    var from = $('#event_start_date').datetimepicker("getDate");
                    var to = $('#event_end_date').datetimepicker("getDate");
                    if (from == null || from > to) {
                        alert({
                            content: self.options.eventdateMessage
                        });
                        $('#event_end_date').val('');
                    }
                }
            ...
            $('#save-btn').click(function (e) {
                if ($("#form-seller-event-new").valid()!==false) {
        ...
        callVerifySkuAjaxFunction: function () {
            var self = this;
            let productId = parseInt($('input[name=product_id]').val());
            if (productId==NaN) {
                productId = 0;
            }
            $.ajax({
                ...
                type: "POST",
                data: {sku:$('input#sku').val(),'product_id':productId},
                dataType: 'html',
    ...
    return $.mage.sellerAddProductjs;
});
