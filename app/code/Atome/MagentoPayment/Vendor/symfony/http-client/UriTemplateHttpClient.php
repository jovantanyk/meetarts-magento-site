...
class UriTemplateHttpClient implements HttpClientInterface, ResetInterface
{
    ...
    {
        if (\class_exists(\Atome\MagentoPayment\Vendor\GuzzleHttp\UriTemplate\UriTemplate::class)) {
            return \Atome\MagentoPayment\Vendor\GuzzleHttp\UriTemplate\UriTemplate::expand(...);
        }
        if (\class_exists(\Atome\MagentoPayment\Vendor\League\Uri\UriTemplate::class)) {
            return static fn(string $url, array $vars): string => (new \Atome\MagentoPayment\Vendor\League\Uri\UriTemplate($url))->expand($vars);
        }
        if (\class_exists(\Atome\MagentoPayment\Vendor\Rize\UriTemplate::class)) {
            return (new \Atome\MagentoPayment\Vendor\Rize\UriTemplate())->expand(...);
        }
