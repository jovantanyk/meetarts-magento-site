...
namespace Atome\MagentoPayment\Vendor\Sentry\Serializer;
...
use Atome\MagentoPayment\Vendor\Sentry\ExceptionDataBag;
use Atome\MagentoPayment\Vendor\Sentry\Frame;
use Atome\MagentoPayment\Vendor\Sentry\Options;
use Atome\MagentoPayment\Vendor\Sentry\Profiling\Profile;
use Atome\MagentoPayment\Vendor\Sentry\Tracing\DynamicSamplingContext;
use Atome\MagentoPayment\Vendor\Sentry\Tracing\Span;
use Atome\MagentoPayment\Vendor\Sentry\Tracing\TransactionMetadata;
use Atome\MagentoPayment\Vendor\Sentry\Util\JSON;
/**
...
final class PayloadSerializer implements PayloadSerializerInterface
{
    ...
    public function toArray(Event $event) : array
    {
         ...
         */
        if (EventType::event() === $event->getType() && !$this->options->isTracingEnabled()) {
            ...
            if ($dynamicSamplingContext instanceof DynamicSamplingContext) {
                ...
                if (null !== $replayId) {
                    $result['contexts']['replay'] = ['replay_id' => $replayId];
                }
            }
        }
