...
use Atome\MagentoPayment\Vendor\GuzzleHttp\RequestOptions as GuzzleHttpClientOptions;
use Atome\MagentoPayment\Vendor\Http\Adapter\Guzzle6\Client as Guzzle6HttpClient;
use Atome\MagentoPayment\Vendor\Http\Adapter\Guzzle7\Client as Guzzle7HttpClient;
use Atome\MagentoPayment\Vendor\Http\Client\Common\Plugin\AuthenticationPlugin;
use Atome\MagentoPayment\Vendor\Http\Client\Common\Plugin\DecoderPlugin;
use Atome\MagentoPayment\Vendor\Http\Client\Common\Plugin\ErrorPlugin;
use Atome\MagentoPayment\Vendor\Http\Client\Common\Plugin\HeaderSetPlugin;
use Atome\MagentoPayment\Vendor\Http\Client\Common\Plugin\RetryPlugin;
use Atome\MagentoPayment\Vendor\Http\Client\Common\PluginClient;
use Atome\MagentoPayment\Vendor\Http\Client\Curl\Client as CurlHttpClient;
use Atome\MagentoPayment\Vendor\Http\Client\HttpAsyncClient as HttpAsyncClientInterface;
use Atome\MagentoPayment\Vendor\Http\Discovery\HttpAsyncClientDiscovery;
...
use Atome\MagentoPayment\Vendor\Sentry\HttpClient\Authentication\SentryAuthentication;
use Atome\MagentoPayment\Vendor\Sentry\HttpClient\Plugin\GzipEncoderPlugin;
use Atome\MagentoPayment\Vendor\Sentry\Options;
use Atome\MagentoPayment\Vendor\Symfony\Component\HttpClient\HttpClient as SymfonyHttpClient;
use Atome\MagentoPayment\Vendor\Symfony\Component\HttpClient\HttplugClient as SymfonyHttplugClient;
/**
...
final class HttpClientFactory implements HttpClientFactoryInterface
{
    ...
    private function resolveClient(Options $options)
    {
        if (\class_exists(SymfonyHttplugClient::class)) {
            $symfonyConfig = ['timeout' => $options->getHttpConnectTimeout(), 'max_duration' => $options->getHttpTimeout(), 'http_version' => $options->isCompressionEnabled() ? '1.1' : null];
        ...
        if (\class_exists(CurlHttpClient::class)) {
            $curlConfig = [\CURLOPT_TIMEOUT => $options->getHttpTimeout(), \CURLOPT_HTTP_VERSION => $options->isCompressionEnabled() ? \CURL_HTTP_VERSION_1_1 : \CURL_HTTP_VERSION_NONE, \CURLOPT_CONNECTTIMEOUT => $options->getHttpConnectTimeout()];
            if (null !== $options->getHttpProxy()) {
