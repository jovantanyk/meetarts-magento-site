...
namespace Atome\MagentoPayment\Vendor\Sentry\Tracing;

use Closure;
use Atome\MagentoPayment\Vendor\GuzzleHttp\Exception\RequestException as GuzzleRequestException;
...
use Atome\MagentoPayment\Vendor\Sentry\ClientInterface;
use Atome\MagentoPayment\Vendor\Sentry\SentrySdk;
use Atome\MagentoPayment\Vendor\Sentry\State\HubInterface;
use function Atome\MagentoPayment\Vendor\Sentry\getBaggage;
use function Atome\MagentoPayment\Vendor\Sentry\getTraceparent;
/**
...
final class GuzzleTracingMiddleware
{
            ...
            return static function (RequestInterface $request, array $options) use($hub, $handler) {
                ...
                if (null === $span) {
                    if (self::shouldAttachTracingHeaders($client, $request)) {
                        $request = $request->withHeader('sentry-trace', getTraceparent())->withHeader('baggage', getBaggage());
                    }
    ...
    private static function shouldAttachTracingHeaders(?ClientInterface $client, RequestInterface $request) : bool
    {
        if (null !== $client) {
            ...
            if (null !== $sdkOptions->getTracePropagationTargets() && ([] === $sdkOptions->getTracePropagationTargets() || \in_array($request->getUri()->getHost(), $sdkOptions->getTracePropagationTargets()))) {
                return \true;
            }
        }
        return \false;
